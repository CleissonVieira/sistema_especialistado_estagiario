...
class Pergunta:
	def __init__(self):
		...
		['alimentacao_conectada', 'A alimentacao esta conectada?'],
		['voltagens_corretas', 'A voltagem esta correta?'],
		['a_bios_esta_iniciando', 'A BIOS esta iniciando ?'],
		['speaker_emite_alertas', 'O speaker emite alertas?'],
		['boot_os', 'Boot OS?'],
		['unidade_de_armazenamento_detectada', 'A unidade de aramazenamento eh detectada?'],
		['ordem_de_boot_correta', 'A ordem de boot esta correta?'],	
		['unidade_de_armazenamento_integra', 'A unidade de armazenamento esta integra?'],
		['os_inicia_completamente', 'OS inicia completamente?'],
		['funcionamento_normal', 'Fique feliz, porque seu computador esta funcionando perfeitamente! :)'],
		['problema_na_placa_mae', 'Deu ruim, deu problema na placa mae. Procure uma assistencia tecnica.'],
		['checar_processador_e_memoria_ram', 'Deu ruim, problema no processador ou memória ram. Troque a pasta térmica e passe borracha na memoria ram.'],
